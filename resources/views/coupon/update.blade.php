...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Update Coupon') }}</div>
                ...
                <div class="card-body">
                    <form method="POST" action="" enctype="multipart/form-data" >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input type="text" id="name" name="Coupon[name]" autofocus class="form-control"
                                    value="{{ !empty(old('Coupon')['name']) ? old('Coupon')['name'] : $coupon->name }}" />
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input class="date form-control" type="text" name="Coupon[start_date]" id="start_date"
                                    default="start_date"
                                    value="{{ !empty(old('Coupon')['start_date']) ? old('Coupon')['start_date'] : date('Y-m-d', strtotime($coupon->start_date)) }}" autocomplete = "off">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input class="date form-control" type="text" name="Coupon[end_date]" id="end_date"
                                    value="{{ !empty(old('Coupon')['end_date']) ? old('Coupon')['end_date'] : date('Y-m-d', strtotime($coupon->end_date)) }}" autocomplete = "off">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                                ...
                                @foreach($products as $product)
                                    ...
                                    @if(!empty($coupon->products) && count($coupon->products) > 0)
                                        @foreach($coupon->products as $productSelect)
                                            @if($productSelect->product_id == $product->product_id)
                                                @php
                                                    $selected = true;
                                                    break;
                                                @endphp
                            ...
                            <input type='hidden' id="product_ids_select" name="Coupon[product_ids]" value="{{ !empty(old('Coupon')['products']) ? old('Coupon')['product_ids'] : $coupon->product_ids }}" />
                            </div>
                        </div>
